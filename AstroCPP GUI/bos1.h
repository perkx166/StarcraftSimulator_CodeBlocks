...
void rysuj(){
    readimagefile("bos1.bmp",x-50,y-35,x+50,y+40);
    xlt=x-70;
    xpt=x+70;
    yt=y+10;
};
...
if(ys>0){
    ys=ys+5+wsp;
    strzalr(xs+45,ys+47,strzalpexist);
    strzalr(xs-45,ys+47,strzallexist);
    strzalr(xs,ys+47,strzalsexist);
}
time++;
}
};
...
void mechanikastrzaluu(){
    strzalu(xs+45,ys+47,strzalpexist);
    strzalu(xs-45,ys+47,strzallexist);
    strzalu(xs,ys+47,strzalsexist);
};
...
int strzalsexist; /** czy strzal prawy istnieje **/
